...
| --------- | --------------------- | ------------- |
| **Name** | `DN_ENDPOINT_{n}_NAME` | Name of the endpoint (required, must be unique) |
| **Template** | `DN_ENDPOINT_{n}_TEMPLATE` | Payload template name used by the website to parse the response into notifications (required) |
...
| **Method** | `DN_ENDPOINT_{n}_METHOD` | HTTP method: GET, POST, PUT, DELETE, PATCH, HEAD, OPTIONS |
| **URL** | `DN_ENDPOINT_{n}_URL` | The URL to fetch (must begin with `http://` or `https://`) |
| **Body** | `DN_ENDPOINT_{n}_BODY` | Request body for POST/PUT/PATCH requests |
| **Query** | `DN_ENDPOINT_{n}_QUERY` | Query parameters, one `key=value` per line |
| **Headers** | `DN_ENDPOINT_{n}_HEADER` | HTTP headers, one `key: value` per line |
